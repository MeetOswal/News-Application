...
class Model:
    def __init__(self):
        ...
        - Load BART summarization and encoder models.
    ...
    def transform(self, raw_data, article_collection):
        """
        Transform raw input data to Hugging Face Dataset:
        - Filter out already existing URIs.
        - Parse article metadata and extract concepts.
        - Convert raw JSON into structured dataset.
        """
        for row in raw_data:
    ...
    def summarize(self):
        ...
        - Combine summaries and generate BART-based mean-pool embeddings.
        - Append summaries and embeddings to dataset.
        """
        def split_text_into_chunks(text, max_tokens):
            """
            Split text into chunks based on max token length using tokenizer.
            """
        ...
        def summarize_article(article):
            """
            Generate summary for a single article split into token chunks.
            """
        ...
        def summary_tokenizer(summary):
            """
            Tokenize and embed a summary using BART encoder (mean pooling).
            """
        ...
        def batch_summarize(batch):
            """
            Apply summarization across a batch of articles.
            """
    ...
    def insert_to_mongo(self, keywords_col, articles_col):
        """
        Insert article metadata, summaries, and embeddings into MongoDB.
        Also, insert keyword skeletons for time-series tracking.
        """
        documents = []
        keywords = []
    ...
    def insert_to_qdrant(self, client, collection_name):
        """
        Insert article embeddings to Qdrant vector database with metadata.
        """
        try:
    ...
    def search(self, client, collection, query):
        """
        Perform semantic search on Qdrant using query embedding.
        Fetch related documents from MongoDB.
        Generate a final answer using BART on summaries and the query.
        """
        ...
        def embed_query(query):
            """
            Generate mean-pooled BART embedding for a query string.
            """
